...
def _period_to_year_month(period: str) -> tuple[int, int]:
    """
    Преобразует период YYYY-MM или YYYY-MM-DD в пару (year, month).
    """
    ...
    if len(parts) < 2:
        raise ValueError("period должен быть в формате YYYY-MM или YYYY-MM-DD")

    year = int(parts[0])
    month = int(parts[1])

    if not (1 <= month <= 12):
        raise ValueError(f"month={month} вне диапазона 1..12")
...
def _period_to_date(period: str) -> str:
    """
    Преобразует YYYY-MM в YYYY-MM-01.
    Если дата уже YYYY-MM-DD, возвращает её в нормализованном виде.
    """
    if not isinstance(period, str):
        raise ValueError("period должен быть строкой YYYY-MM или YYYY-MM-DD")
    ...
    if len(parts) == 3:
        ...
        if not (1 <= month <= 12):
            raise ValueError(f"month={month} вне диапазона 1..12")
        if not (1 <= day <= 31):
            raise ValueError(f"day={day} вне диапазона 1..31")
...
def _extract_metric_value(metric: str, period: str) -> dict:
    """
    Получить значение одной метрики за один период через существующие инструменты.
    """
    metric = metric.strip()

    if metric == "key_rate":
        on_date = _period_to_date(period)
        obs = get_key_rate(on_date=on_date)
        if "error" in obs:
            return obs

        return {
            "date": obs.get("date", on_date),
    ...
    if metric.startswith("fx_"):
        currency = metric.replace("fx_", "", 1).upper()
        on_date = _period_to_date(period)
        obs = get_fx_rate(currency=currency, on_date=on_date)
        if "error" in obs:
            return obs
    ...
    if metric == "cpi":
        year, month = _period_to_year_month(period)
        obs = get_inflation(year=year, month=month)
        if "error" in obs:
            return obs
    ...
    if metric == "unemployment":
        year, month = _period_to_year_month(period)
        obs = get_unemployment(year=year, month=month)
        if "error" in obs:
            return obs
    ...
    return {
        "error": (
            "неизвестная метрика. Доступны: key_rate, fx_USD, fx_EUR, "
...
def compare_periods(metric: str, period_a: str, period_b: str) -> dict:
    ...
    Args:
        ...
        period_a: первый период, YYYY-MM или YYYY-MM-DD
        period_b: второй период, YYYY-MM или YYYY-MM-DD
    ...
    """
    try:
        a = _extract_metric_value(metric, period_a)
        if "error" in a:
            return {
                "metric": metric,
                "period_a": period_a,
        ...
        b = _extract_metric_value(metric, period_b)
        if "error" in b:
            return {
                "metric": metric,
                "period_a": period_a,
        ...
        return {
            ...
            "ratio": ratio,
            "source": f"{a.get('source')}|{b.get('source')}",
        }
