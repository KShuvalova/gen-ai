...
def _append_jsonl(path: Path, row: dict[str, Any]) -> None:
    """Добавить одну строку JSONL с логом запуска агента."""
...
ВАЖНОЕ ПРАВИЛО ДЛЯ СРАВНЕНИЙ: если пользователь просит сравнить одну и ту же метрику между двумя датами или периодами, спрашивает «во сколько раз изменилось», «на сколько изменилось», «выросло/снизилось с ... по ...», ОБЯЗАТЕЛЬНО сначала используй compare_periods. Не заменяй такой запрос двумя отдельными вызовами базового инструмента.\n\nИнструменты:
...
- compare_periods: сравнение одной метрики в двух периодах
...
6. Кросс-курс «сколько B за 1 A» = (рублей за 1 A) / (рублей за 1 B).\n7. Если вопрос просит сравнить одну и ту же метрику между двумя датами или месяцами, используй compare_periods.
...
def main():
    ...
    trace_path = a.trace or Path("trace.jsonl")
    _append_jsonl(
        trace_path,
        {
    ...
    if not a.quiet:
        print(f"(лог сохранён: {trace_path})")
