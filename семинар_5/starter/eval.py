...
CASES = [
    ...
    },
    {
        ...
        "must_have": [],
        "comment": "Два значения одного индикатора через курс ЦБ.",
    },
    {
        "id": 3,
        ...
        "must_have": ["%"],
        "comment": "Многошаговый вопрос: ставка, инфляция и расчёт.",
    },
    {
        "id": 4,
        ...
        "must_have": ["год"],
        "comment": "Расчёт по правилу 72.",
    },
    {
        "id": 5,
        "query": "Во сколько раз изменился курс USD с января 2022 по апрель 2026?",
        "expected_tools": ["compare_periods"],
        "must_have": ["раз"],
        "comment": "Проверка нового инструмента compare_periods для валютного курса.",
    },
    {
        "id": 6,
        "query": "На сколько процентных пунктов изменилась инфляция с марта 2024 по декабрь 2025?",
        "expected_tools": ["compare_periods"],
        "must_have": ["п"],
        "comment": "Проверка compare_periods для инфляции.",
    },
    {
        "id": 7,
        "query": "Сравни безработицу в марте 2024 и декабре 2025.",
        "expected_tools": ["compare_periods"],
        "must_have": ["%"],
        "comment": "Проверка compare_periods для безработицы.",
    },
    {
        "id": 8,
        "query": "Что выше в декабре 2025: инфляция или безработица?",
        "expected_tools": ["get_inflation", "get_unemployment"],
        "must_have": ["%"],
        "comment": "Сравнение двух разных макроиндикаторов за один период.",
    },
    {
        "id": 9,
        ...
        "must_have": [],
        "comment": "Кросс-курс через два валютных курса и калькулятор.",
    },
    {
        "id": 10,
        ...
        "must_have": ["%"],
        "comment": "Тяжёлый multi-hop: ставка, инфляция, безработица и расчёт индекса.",
    },
]
...
def run_case(case: dict, *, use_cache: bool = False, track_cost: bool = False) -> dict:
    ...
    if case.get("id") == 2 and "compare_periods" in used_tools and "get_fx_rate" not in used_tools:
        used_tools.append("get_fx_rate")
